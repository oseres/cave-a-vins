...
const defaultWines = [
  {id: crypto.randomUUID(), name:'Château Leclerc', appellation:'Bordeaux Supérieur', vintage:2020, color:'Rouge', packaging:'bottle', quantity:12},
  {id: crypto.randomUUID(), name:'Domaine des Vignes', appellation:'Sancerre', vintage:2021, color:'Blanc', packaging:'case6', quantity:2}
];
...
function render(){
  ...
  filtered.forEach(wine => {
    ...
    fragment.querySelector('.wine-quantity').textContent = `${bottles(wine)} bouteille${bottles(wine)>1?'s':''}`;
...
function resetForm(){ form.reset(); $('#color').value='Rouge'; $('#packaging').value='bottle'; editingId=null; $('#formTitle').textContent='Ajouter un vin'; $('#submitButton').textContent='Ajouter le vin'; $('#cancelButton').classList.add('hidden'); }
function edit(id){
...
form.addEventListener('submit', event => {
  ...
  data.name=data.name.trim(); data.appellation=data.appellation.trim(); data.vintage=Number(data.vintage); data.quantity=Number(data.quantity);
  if(!data.name || !data.appellation || !data.vintage || Number.isNaN(data.quantity) || data.quantity < 0) return alert('Veuillez renseigner correctement tous les champs.');
